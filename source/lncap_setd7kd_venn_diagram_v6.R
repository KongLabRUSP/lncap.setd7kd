...
g1.1 <- dt1$SYMBOL[dt1$`WT PEITC vs. WT`>= 0.3]
g1.2 <- dt1$SYMBOL[dt1$`KD PEITC vs. KD` <= -0.3]

g1.3 <- dt1$SYMBOL[dt1$`WT PEITC vs. WT`<= -0.3]
g1.4 <- dt1$SYMBOL[dt1$`KD PEITC vs. KD` >= 0.3]

g2.1 <- dt1$SYMBOL[dt1$`KD PEITC vs. KD`>= 0.4]
g2.2 <- dt1$SYMBOL[dt1$`KD vs. WT` <= -0.4]

g2.3 <- dt1$SYMBOL[dt1$`KD PEITC vs. KD`<= -0.4]
g2.4 <- dt1$SYMBOL[dt1$`KD vs. WT` >= 0.4]

# Save the lists
dtt <- qpcR:::cbind.na(`WT PEITC vs. WT Up` = g1.1,
                       `KD PEITC vs. KD Down` = g1.2,
                       `WT PEITC vs. WT Down` = g1.3,
                       `KD PEITC vs. KD Up` = g1.4,
...
# Matched list----
# 1. WT PEITC vs. WT + KD PEITC vs. KD
m1.up.dn <- g1.1[g1.1 %in% g1.2]
m1.dn.up <- g1.3[g1.3 %in% g1.4]

# 2. KD vs. WT + KD PEITC vs. KD
m2.up.dn <- g2.1[g2.1 %in% g2.2]
m2.dn.up <- g2.3[g2.3 %in% g2.4]

dtt2 <- qpcR:::cbind.na(`WT PEITC vs. WT Up + KD PEITC vs. KD Down` = m1.up.dn,
                       `WT PEITC vs. WT Down + KD PEITC vs. KD Up` = m1.dn.up,
                       `KD PEITC vs. KD Up + KD vs. WT Down` = m2.up.dn,
                       `KD PEITC vs. KD Down + KD vs. WT Up` = m2.dn.up)
write.csv(dtt2,
          file = "tmp/gene_overlap_list.csv",
          row.names = FALSE)
